...
import { detecType } from "./helpers.js";
...
/* Kullanıcının konumunu öğrenmek için getCurrentPositon metodunu kullandık ve bizden iki parametre istedi:

/*1. Kullanıcı konum iznini verdiğimizde çalışacak fonksiyondur.
  2.  Kullanıcı konum iznini vermediğimizde çalışacak fonksiyondur.
*/
...
function loadMap(e) {
  ...
  map = L.map("map").setView([e.coords.latitude, e.coords.latitude], 10);
  L.control;

  //! 2.Hritanın nasıl görüneceğini belirler
...
function renderMarker(item) {
  // L.marker([50.505], { icon: myIcon}).addTo(map);
  // Markerı oluşturur
...
//! notes alanında tıklama olayını izler
function handleClick(e) {
  ...
  if (e.target.id === "delete") {
    //* idisini bildiğimiz elemanı diziden filter yöntemi kullanarak kaldırdık.
  ...
  if ((e.target.id = "fly")) {
    //* tıkladığımız elemanın id si ile dizi içerisindeki elemanlardan herhangi birinin id si eşlesirse bul.
